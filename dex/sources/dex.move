...
module dex::dex {
    ...
    public fun user_last_mint_epoch<CoinType>(self: &Storage, user: address): u64 {
        ...
        if(table::contains(&data.faucet_lock, user)){
            return *table::borrow(&data.faucet_lock, user)
        };
        0
    }

    public fun user_swap_count(self: &Storage, user: address): u64 {
        if(table::contains(&self.swaps, user)){
            return *table::borrow(&self.swaps, user)
        };
        0
    }
    ...
    public fun entry_place_market_order(
        ...
        let (eth, usdc, coin_dex) = place_market_order(
                self,
                pool,
                account_cap,
                quantity,
    ...
    public fun place_market_order(
        ...
        let (eth_coin, usdc_coin) = clob::place_market_order<ETH, USDC>(pool, account_cap, client_order_id, quantity, is_bid, base_coin, quote_coin, c, ctx);
    ...
    public fun fill_pool(
        ...
        create_ask_order(self, pool, c, ctx);
        create_bid_order(self, pool, c, ctx);
    }
    ...
    public fun create_state(
     ...
     public fun mint_coin<CoinType>(self: &mut Storage, ctx: &mut TxContext): Coin<CoinType> {
        ...
        let type_name = get<CoinType>();
        let data = df::borrow_mut<TypeName, Data<CoinType>>(&mut self.id, type_name);
        ...
        *last_mint_epoch = tx_context::epoch(ctx);
        coin::mint(&mut data.cap, if (type_name == get<USDC>()) 100 * FLOAT_SCALING else 1 * FLOAT_SCALING, ctx)
    }
